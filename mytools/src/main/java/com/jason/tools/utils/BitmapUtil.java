...
import com.jason.tools.encrypt.Base64Util;
...
public final class BitmapUtil {
    ...
    public static Bitmap createBitmap(Context context, byte[] data, float orientation) {
        ...
        try {
            ...
            int hight = DensityUtil.getDisplayHeight(context);
            int min = Math.min(width, hight);
            opts.inJustDecodeBounds = true;
            BitmapFactory.decodeByteArray(data, 0, data.length, opts);
            opts.inSampleSize =
                    BitmapUtil.computeSampleSize(opts, min, BitmapUtil.MAXLENTH * BitmapUtil.MAXLENTH);
            opts.inJustDecodeBounds = false;
            bitmap = BitmapFactory.decodeByteArray(data, 0, data.length, opts);
            transformed = BitmapUtil.rotateBitmap(orientation, bitmap);
            ...
            BitmapFactory.decodeByteArray(data, 0, data.length, opts);
            opts.inSampleSize =
                    BitmapUtil.computeSampleSize(opts, -1, opts.outWidth * opts.outHeight
                            / BitmapUtil.PIC_COMPRESS_SIZE);
            opts.inJustDecodeBounds = false;
            bitmap = BitmapFactory.decodeByteArray(data, 0, data.length, opts);
            transformed = BitmapUtil.rotateBitmap(orientation, bitmap);
        }
    ...
    public static int computeSampleSize(BitmapFactory.Options options, int minSideLength, int maxNumOfPixels) {
        int initialSize = BitmapUtil.computeInitialSampleSize(options, minSideLength, maxNumOfPixels);
        int roundedSize;
    ...
    public static int computeInitialSampleSize(BitmapFactory.Options options, int minSideLength,
        ...
        int upperBound =
                (minSideLength == -1) ? BitmapUtil.IMAGEBOUND : (int) Math.min(
    ...
    public static String bitmapToJpegBase64(Bitmap bitmap, int quality, float maxSize) {
        try {
            ...
            return Base64Util.encodeToString(data, Base64Util.NO_WRAP);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public static Bitmap createBitmap(Context context, String filename, int orientatoin) {
        ...
        try {
            ...
            transformed = BitmapUtil.rotateBitmap(orientatoin, bitmap);
        } catch (OutOfMemoryError e) {
            e.printStackTrace();
            if (bitmap != null) {
                bitmap.recycle();
